    ...
    async.waterfall([
        function (callback) {
          request(endpoints.customersUrl + "/" + custId, function (error, response, body) {
            ...
            var addressLink = endpoints.customersUrl + "/" + custId + "/addresses";
            var cardLink = endpoints.customersUrl + "/" + custId + "/cards";
            var order = {
              ...
              function (callback) {
                ...
                request.get(addressLink, function (error, response, body) {
                  ...
                  if (jsonBody.status_code !== 500 && jsonBody._embedded.address[0] != null) {
                    order.address = endpoints.addressUrl + "/" + jsonBody._embedded.address[0].id;
                  }
                  callback();
                });
              },
              function (callback) {
                ...
                request.get(cardLink, function (error, response, body) {
                  ...
                  if (jsonBody.status_code !== 500 && jsonBody._embedded.card[0] != null) {
                    order.card = endpoints.cardsUrl + "/" + jsonBody._embedded.card[0].id;
                  }
                  callback();
                });
              }
